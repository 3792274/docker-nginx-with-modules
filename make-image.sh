...
docker build --build-arg ENABLED_MODULES="echo fancy stream-echo ndk lua auth-spnego brotli encrypted-session fips-check geoip geoip2 headers-more image-filter modsecurity njs opentracing passenger perl rtmp set-misc subs-filter xslt" -f Dockerfile.alpine -t $USERID/nginx . &&
